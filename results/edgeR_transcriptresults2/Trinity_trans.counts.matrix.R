...
if (file.exists("Trinity_trans.counts.matrix.RData") && ! NO_REUSE) {
    print('RESTORING DATA FROM EARLIER ANALYSIS')
    load("Trinity_trans.counts.matrix.RData")
} else {
    print('Reading matrix file.')
    primary_data = read.table("Trinity_trans.counts.matrix", header=T, com='', row.names=1, check.names=F, sep='\t')
    primary_data = as.matrix(primary_data)
}
source("/home/anabwire/miniconda3/envs/trinity/opt/trinity-2.8.5/Analysis/DifferentialExpression/R/heatmap.3.R")
source("/home/anabwire/miniconda3/envs/trinity/opt/trinity-2.8.5/Analysis/DifferentialExpression/R/misc_rnaseq_funcs.R")
source("/home/anabwire/miniconda3/envs/trinity/opt/trinity-2.8.5/Analysis/DifferentialExpression/R/pairs3.R")
source("/home/anabwire/miniconda3/envs/trinity/opt/trinity-2.8.5/Analysis/DifferentialExpression/R/vioplot2.R")
data = primary_data
...
write.table(data, file="Trinity_trans.counts.matrix.minRow10.CPM.log2.centered.dat", quote=F, sep='	');
pdf("Trinity_trans.counts.matrix.minRow10.CPM.log2.centered.prcomp.principal_components.pdf")
prin_comp_data = data
...
write.table(pca$rotation, file="Trinity_trans.counts.matrix.minRow10.CPM.log2.centered.PCA.prcomp.scores", quote=F, sep="	")
write.table(pca$x, file="Trinity_trans.counts.matrix.minRow10.CPM.log2.centered.PCA.prcomp.loadings", quote=F, sep="	")
PCA.loadings=pca$x
